...
const DEMO_REHYDRATION_KEY = "optimile.platform.demoTenantsRehydrated.v3";
const DEMO_TENANT_IDS = ["tenant-easylane", "tenant-nippon01", "tenant-easylane-cargo"] as const;
...
function ensureDemoTenantsRehydrated(): void {
  ...
  try {
    ...
    const storedTenants: TenantRecord[] = storedTenantsRaw ? JSON.parse(storedTenantsRaw) : [];
    const existingIds = new Set(storedTenants.map((tenant) => tenant.id));
    const missingTenants = mockPlatformTenants.filter(
      (tenant) =>
        DEMO_TENANT_IDS.includes(tenant.id as (typeof DEMO_TENANT_IDS)[number]) &&
        !existingIds.has(tenant.id),
    );
    if (missingTenants.length > 0) {
      window.localStorage.setItem(
        storageKeys.platformTenants,
        JSON.stringify([...storedTenants, ...missingTenants]),
      );
    }

    // Hierarchy / workspaces
    const storedWorkspacesRaw = window.localStorage.getItem(storageKeys.tenantWorkspaces);
    const storedWorkspaces: Record<string, TenantWorkspaceState> = storedWorkspacesRaw
      ? JSON.parse(storedWorkspacesRaw)
      : {};
    let workspacesChanged = false;
    for (const tenantId of DEMO_TENANT_IDS) {
      if (storedWorkspaces[tenantId]) continue;
      const tenant = mockPlatformTenants.find((item) => item.id === tenantId);
      if (!tenant) continue;
      storedWorkspaces[tenantId] = {
        tenantId,
        ...
        hierarchy: {
          ...
          startingBlueprint: tenant.initialHierarchyTemplate as HierarchyTemplateCode,
          levels: mockHierarchyLevels
            .filter((level) => level.tenantId === tenantId)
            .sort((a, b) => a.order - b.order),
          lastUpdated: new Date().toISOString(),
        },
      };
    ...
    if (workspacesChanged) {
      window.localStorage.setItem(storageKeys.tenantWorkspaces, JSON.stringify(storedWorkspaces));
    }

    // Roles
    const storedRolesRaw = window.localStorage.getItem(storageKeys.tenantRoles);
    const storedRoles: RoleDefinition[] = storedRolesRaw ? JSON.parse(storedRolesRaw) : [];
    const existingRoleIds = new Set(storedRoles.map((role) => role.id));
    const missingRoles = mockRoles.filter(
      (role) =>
        DEMO_TENANT_IDS.includes(role.tenantId as (typeof DEMO_TENANT_IDS)[number]) &&
        !existingRoleIds.has(role.id),
    );
    if (missingRoles.length > 0) {
      window.localStorage.setItem(
        storageKeys.tenantRoles,
        JSON.stringify([...storedRoles, ...missingRoles]),
      );
    }

    // Users
    const storedUsersRaw = window.localStorage.getItem(storageKeys.tenantUsers);
    const storedUsers: UserRecord[] = storedUsersRaw ? JSON.parse(storedUsersRaw) : [];
    const existingUserIds = new Set(storedUsers.map((user) => user.id));
    const missingUsers = mockUsers.filter(
      (user) =>
        DEMO_TENANT_IDS.includes(user.tenantId as (typeof DEMO_TENANT_IDS)[number]) &&
        !existingUserIds.has(user.id),
    );
    if (missingUsers.length > 0) {
      window.localStorage.setItem(
        storageKeys.tenantUsers,
        JSON.stringify([...storedUsers, ...missingUsers]),
      );
    }

    // Role-permission matrix — feature-level merge of the demo grants. Only
    ...
    Object.entries(mockRolePermissionMatrixSeed).forEach(([roleId, moduleMap]) => {
      const roleEntry = storedMatrix[roleId] ?? {};
      Object.entries(moduleMap as Record<string, Record<string, Record<string, boolean>>>).forEach(
        ([moduleCode, featureMap]) => {
          const moduleEntry = roleEntry[moduleCode] ?? {};
          Object.entries(featureMap).forEach(([featureCode, actions]) => {
            if (moduleEntry[featureCode] === undefined) {
              moduleEntry[featureCode] = { ...actions };
              matrixChanged = true;
            }
          });
          roleEntry[moduleCode] = moduleEntry;
        },
      );
      storedMatrix[roleId] = roleEntry;
    });
    if (matrixChanged) {
      window.localStorage.setItem("optimile.tenant.rolePermissionMatrix", JSON.stringify(storedMatrix));
    }
...
function ensureRequiredPlatformModules(modules: PlatformModule[]) {
  ...
  requiredByCode.forEach((requiredModule, code) => {
    ...
    // Canonical seed wins for descriptive metadata (name, category,
    // description, folderPath, startRoute) so the registry stays in sync
    // with the real module folders. Only the admin's stored id + status
    // (enable/disable choice) are preserved.
...
function validateUserAssignments(
  ...
  // Tenant Admin is system-managed and never bound to an org unit. Match
  // by id pattern OR canonical name OR ALL_TENANT scope so the rule
  // survives any legacy data where the stored dataScope drifted.
  const normalisedRoleName = role.name.trim().toLowerCase();
  const isTenantAdmin =
    role.id.endsWith("-tenant-admin") ||
    normalisedRoleName === "tenant admin" ||
    normalisedRoleName === "tenant admin / ceo" ||
  ...
  // user can be saved unscoped and assigned later when units exist. We
  // only enforce a selection when there's something to choose from.
