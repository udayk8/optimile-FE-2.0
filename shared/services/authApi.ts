...
export const authApi = {
  ...
  async refresh(refreshToken: string): Promise<AuthTokenResponseDto> {
    try {
      return await apiClient.post<AuthTokenResponseDto>('/api/v1/auth/refresh', { refreshToken }, { auth: false, retryOnAuthFailure: false });
    } catch (error) {
      if (error instanceof ApiError && (error.status === 403 || error.status === 404 || error.status === 405)) {
        return apiClient.post<AuthTokenResponseDto>('/auth/refresh', { refreshToken }, { auth: false, retryOnAuthFailure: false });
      }
      throw error;
    }
  },
