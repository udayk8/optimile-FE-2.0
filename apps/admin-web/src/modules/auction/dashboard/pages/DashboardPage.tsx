...
import { HeroCard } from '@admin/components/cards/HeroCard'
import { KPICard } from '@admin/components/cards/KPICard'
...
import { MOCK_DASHBOARD } from '@admin/utils/mock-data'
      ...
      <HeroCard
        ...
        action={
          <Button asChild variant="outline">
            <Link to="/admin/auction/auctions">Review Auctions</Link>
          </Button>
        }
      />
        ...
        <Card>
          <CardHeader className="flex-row items-center justify-between space-y-0">
            ...
            <Button asChild size="sm" variant="outline">
              <Link to="/admin/auction/auctions">View all</Link>
            </Button>
          </CardHeader>
          <CardContent className="space-y-4">
            {auctions.slice(0, 4).map((auction) => (
              <Link
                key={auction.id}
                to={`/admin/auction/auctions/${auction.id}`}
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            {expiringContracts.map((contract) => (
              <Link
                key={contract.id}
                to={`/admin/auction/contracts/${contract.id}`}
