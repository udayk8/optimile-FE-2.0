            ...
            <Card key={v.id} className="cursor-pointer hover:border-primary/30" onClick={() => navigate(`/vendor/fleet/vehicles/${v.id}`)}>
              <CardContent className="p-5">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{v.vehicleType}</p>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                  <div><span className="text-muted-foreground">Base: </span>{v.baseLocation}</div>
                  <div><span className="text-muted-foreground">Compliance: </span><StatusBadge status={v.complianceStatus} /></div>
                  <div><span className="text-muted-foreground">GPS: </span>{v.gpsDeviceId || '—'}</div>
                  <div><span className="text-muted-foreground">Docs: </span>{v.complianceDocuments.length}</div>
                </div>
                {v.complianceStatus === 'EXPIRED' && (
                  <div className="mt-3 p-2 bg-red-50 dark:bg-red-900/10 rounded-md text-xs text-red-700 dark:text-red-400 flex items-center gap-2">
          ...
          {drivers.map((d) => (
            <Card key={d.id} className="cursor-pointer hover:border-primary/30" onClick={() => navigate(`/vendor/fleet/drivers/${d.id}`)}>
              <CardContent className="p-5">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                  <div><span className="text-muted-foreground">License: </span><span className="font-mono text-xs">{d.licenseNumber}</span></div>
                  <div><span className="text-muted-foreground">Expiry: </span>{formatDate(d.licenseExpiry)}</div>
                  <div><span className="text-muted-foreground">Class: </span>{d.licenseClass.join(', ')}</div>
                  <div><span className="text-muted-foreground">Docs: </span>{d.complianceDocuments.length}</div>
                </div>
              </CardContent>
            </Card>
          ))}
