          ...
          <div className="space-y-4">
            {filteredInvoices.length === 0 ? (
              ...
              filteredInvoices.map((inv) => (
                <Card key={inv.id} className="cursor-pointer hover:border-primary/30">
                  <CardContent className="p-5">
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        <Button size="sm" variant="ghost" onClick={(e) => { e.stopPropagation(); alert('Downloading Invoice PDF...') }}><Download className="h-3.5 w-3.5" /></Button>
                      </div>
                    </div>
            ...
            <tbody>
              {ledger.map((entry) => (
                <tr key={entry.id} className="border-b hover:bg-accent/50">
                  ...
                  <td className="p-3 text-center">{entry.documentUrl && <Button size="sm" variant="ghost" onClick={() => alert('Downloading Receipt...')}><Download className="h-3.5 w-3.5" /></Button>}</td>
                </tr>
              ))}
            </tbody>
          </table>
