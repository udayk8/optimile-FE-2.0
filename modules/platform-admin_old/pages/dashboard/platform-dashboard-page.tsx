                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        {tenant.code} Â· {tenant.region} Â· {tenant.health.auditEvents24h} audit events in 24h
                      </p>
                    </div>
                    <Badge
                      variant={
        ...
        >
          <PlatformInfoList
            items={planDistribution.map((plan) => ({
              label: plan.name,
              helper: `${plan.code} Â· $${plan.monthlyPriceUsd}/mo Â· ${plan.seatsIncluded} seats`,
              value: (
                ...
                <div key={tenant.id} className="rounded-2xl border bg-background/80 p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <p className="font-medium">{tenant.name}</p>
                      <p className="mt-1 text-sm text-muted-foreground">
                        {tenant.code} Â· {formatTimestamp(tenant.createdAt)}
                      </p>
        ...
        >
          <PlatformTimeline
            items={recentAudit.map((event) => ({
              ...
              meta: `${event.actor} Â· ${formatTimestamp(event.timestamp)}`,
              badge: (
                <Badge
                  variant={
