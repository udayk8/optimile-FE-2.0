...
} from "@tms-booking/modules/tenant-admin/lib/manual-lr";
...
export function TenantManualLRConfigPage() {
  ...
  const availableTargetLevels = useMemo(() => {
    if (currentGovernanceLevel && !canConfigureImmediateChildFromParent) {
      return [];
    }
  ...
  }, [canConfigureImmediateChildFromParent, currentGovernanceLevel, hierarchyLevels]);
  ...
  const inheritedDistributionLevel =
    form.scopeType === "HIERARCHY" && form.ownershipLevelId
      ...
      : null;
  const managedTargetLevel = currentGovernanceLevel ? visibleTargetLevel ?? null : inheritedDistributionLevel;
        ...
        <div className="grid gap-3 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="grid gap-3">
            <div className="grid gap-2 sm:grid-cols-3">
              <CompactInfoCard label="Root" value="Tenant / Company Level" />
            ...
            <div className="grid gap-2 sm:grid-cols-[1.1fr_0.9fr]">
              <Field label="Distribute Up To">
                <Select
                  ...
                  disabled={!canEdit || !canConfigureImmediateChildFromParent}
                >
              ...
              <CompactInfoCard label="Flow" value={`${currentScopeLabel} -> ${distributionTargetLabel}`} />
            </div>
