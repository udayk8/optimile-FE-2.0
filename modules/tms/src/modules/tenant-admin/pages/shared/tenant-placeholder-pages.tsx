...
import { useTenantAuditLogs } from "@tms-booking/modules/tenant-admin/hooks/useTenantAuditLogs";
import { useTenantCustomers } from "@tms-booking/modules/tenant-admin/hooks/useTenantCustomers";
import { useTenantHierarchy } from "@tms-booking/modules/tenant-admin/hooks/useTenantHierarchy";
...
export function TenantUsersPage() {
  ...
  const validRoles = roles.filter((role) => role.active);
  const suggestedRoles = useMemo(
    () => sortRolesByHierarchy(sortRolesForUserType(validRoles, form.userType), levels),
    [form.userType, levels, validRoles],
  );
  const availableRoleOptions = useMemo(
    () => sortRolesByHierarchy(validRoles, levels),
    [levels, validRoles],
  );
  const selectedRole = roles.find((role) => role.id === form.roleId) ?? null;
  const tenantLevelRoleSelected = isTenantLevelRole(selectedRole);
  const allowedOrgUnits = selectedRole && form.userType === "INTERNAL" && !tenantLevelRoleSelected
    ? sortOrgUnitsByHierarchy(
        orgUnits.filter((unit) => unit.hierarchyLevelId === selectedRole.hierarchyLevelId),
        levels,
      )
    : [];
  const mappedLevelName = selectedRole ? getRoleScopeLevelLabel(selectedRole, levelMap) : "";
  ...
  function saveUser() {
    ...
    if (!validRoles.some((role) => role.id === form.roleId)) {
      setMessage("Select an active tenant role.");
      return;
    }
    ...
    if (form.userType === "DRIVER" && !form.linkedDriverId) {
      setMessage("Link a driver record for this driver user.");
      return;
    }

    const linkedDriver = form.userType === "DRIVER" ? drivers.find((driver) => driver.id === form.linkedDriverId) ?? null : null;
    ...
    const payload = {
      ...
      linkedDriverId: form.userType === "DRIVER" ? form.linkedDriverId || null : null,
      driverName: form.userType === "DRIVER" ? linkedDriver?.name ?? form.driverName.trim() : "",
      driverCode: form.userType === "DRIVER" ? linkedDriver?.licenseNumber?.slice(-6) ?? form.driverCode.trim() : "",
      status: form.status,
    };
  ...
  function updateUserType(nextUserType: UserType) {
    setForm((current) => ({
      ...
      linkedDriverId: nextUserType === "DRIVER" ? current.linkedDriverId : "",
  ...
  return (
    <div className="space-y-6">
      ...
      <WorkspaceNote>
        Single role currently supported (multi-role ready). Roles define the hierarchy level type. Users define the actual assigned access scope values from that level. Customer portal users require linked customer plus tenant-level portal enablement. Vendor and driver users remain scoped.
      </WorkspaceNote>
      ...
      {filteredUsers.length ? (
        <DataTable
          ...
          headers={["User", "Role", "Modules", "Allowed level", "Assigned Access Scope", "Status", "Actions"]}
            ...
            user.userType === "INTERNAL"
              ...
              : user.userType === "VENDOR"
                ...
                : user.userType === "CUSTOMER"
                  ...
                  : driverMap.get(user.linkedDriverId ?? "")?.name ?? `${user.driverName || "Driver profile"}${user.driverCode ? ` (${user.driverCode})` : ""}`,
      ...
      >
        <div className="grid gap-4">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Field label="Role" helper="All active tenant roles are shown here. Final access always comes from the selected role permissions.">
              <Select
                value={form.roleId}
                onChange={(event) =>
                  setForm((current) => ({
              ...
              >
                ...
                {availableRoleOptions.map((role) => (
                  <option key={role.id} value={role.id} title={getRoleBrdHoverText(role.name)}>
                    {role.name}{getRoleSuggestionLabel(role, form.userType) ? " - Suggested" : ""}
                  </option>
                ))}
          ...
          <div className="rounded-2xl border bg-muted/20 px-4 py-3 text-sm text-muted-foreground">
            Available active roles for tenant admin assignment:{" "}
            {availableRoleOptions.map((role) => role.name).join(", ") || "No active tenant roles available"}.
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            <Field label="Mapped hierarchy level" helper="This comes directly from the selected role. It defines which org-unit level the user can be assigned from.">
          ...
          <Field label="Role modules" helper="Role modules are shown for visibility. Tenant admin can still assign any active role here.">
          ...
          <Field label="User Permission Override" helper="Future-ready only. Direct user-level permission changes are intentionally disabled in the current frontend flow.">
            <div className="rounded-2xl border border-dashed bg-muted/15 px-4 py-3 text-sm text-muted-foreground">
              Coming later. Current access is controlled by assigned role.
            </div>
          </Field>
          {form.userType === "INTERNAL" ? (
            <Field
              ...
              helper={
                selectedRole
                  ? tenantLevelRoleSelected
                    ? "This role operates at tenant/company level. No org-unit assignment is required."
                    : `This role is mapped to the ${mappedLevelName} hierarchy level. Select one or many actual ${mappedLevelName} org units for this user.`
              ...
              <div className="grid max-h-[320px] gap-3 overflow-y-auto rounded-2xl border bg-muted/20 p-4 md:grid-cols-2">
                {tenantLevelRoleSelected ? (
                  <p className="text-sm text-muted-foreground">
                    Tenant-level role. This user will operate across the tenant without a route-specific org-unit assignment.
                  </p>
          ...
          {form.userType === "CUSTOMER" ? (
            <Field label="Linked customer" helper="Required for customer portal users. They only see records linked to this customer.">
          ...
          {form.userType === "DRIVER" ? (
            <div className="grid gap-4 md:grid-cols-2">
              <Field label="Linked driver" helper="Required for driver users. This controls driver-app scoping and role-based access simulation.">
                <Select
                  value={form.linkedDriverId}
                  onChange={(event) => {
                    const selectedDriver = drivers.find((driver) => driver.id === event.target.value) ?? null;
                    setForm((current) => ({
                      ...current,
                      linkedDriverId: event.target.value,
                      driverName: selectedDriver?.name ?? "",
                      driverCode: selectedDriver?.licenseNumber?.slice(-6) ?? "",
                    }));
                  }}
                >
              ...
              <Field label="Driver reference" helper="Auto-filled from the linked driver record for current frontend-only access simulation.">
                <Input value={form.driverName ? `${form.driverName}${form.driverCode ? ` (${form.driverCode})` : ""}` : "Select a linked driver"} disabled />
              </Field>
            </div>
          ) : null}
          <Field label="Status">
...
export function TenantRolesPage() {
  ...
  const filteredRoles = useMemo(() => {
    const normalizedSearch = search.trim().toLowerCase();
    return sortRolesByHierarchy(roles, levels).filter((role) => {
      if (statusFilter !== "all" && (role.active ? "active" : "inactive") !== statusFilter) {
        return false;
      }
      if (
        normalizedSearch &&
        !`${role.name} ${role.description} ${getRoleScopeLevelLabel(role, levelMap)} ${role.moduleCodes.join(" ")}`
          .toLowerCase()
          .includes(normalizedSearch)
      ) {
        return false;
  ...
  }, [levelMap, levels, roles, search, statusFilter]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Tenant Admin"
        title="Roles"
        description="Tenant-defined roles map to enabled modules and one hierarchy level type. Users then receive actual access-scope values from that level."
        action={<Button onClick={openCreateDialog}>Add Role</Button>}
      />

      <WorkspaceNote>
        Roles must reference enabled modules only. Roles do not store specific org-unit values like South Region or Bangalore Branch. They only define the hierarchy level type allowed for user assignment.
      </WorkspaceNote>
      ...
      {filteredRoles.length ? (
        <DataTable
          title="Roles"
          description="Each role defines the hierarchy level type, module scope, and feature-access responsibility inside this tenant."
          ...
          rows={filteredRoles.map((role) => [
            <div key={`${role.id}-role`} className="min-w-[180px]">
              <p className="font-medium" title={getRoleBrdHoverText(role.name)}>{role.name}</p>
              <p className="mt-1 text-xs text-muted-foreground">
                {formatRoleUserPlaceSummary(users.filter((user) => user.roleId === role.id), orgUnits, {
                  maxUsers: 2,
                  maxPlacesPerUser: 2,
                })}
              </p>
            ...
            getRoleScopeLevelLabel(role, levelMap),
      ...
      <Dialog
        ...
        description="Choose the hierarchy level type and enabled modules for this role, then manage permissions separately."
        widthClassName="max-w-2xl"
        footer={
          <div className="flex justify-end gap-3">
            <Button variant="outline" onClick={() => setOpen(false)}>
      ...
      >
        <div className="grid gap-4">
          <WorkspaceNote>
            This form defines which hierarchy level type the role can operate on and which enabled modules it can access. It does not assign specific org-unit values. Feature permissions are managed on the Role Permissions page. A role must always have at least one enabled module.
          </WorkspaceNote>
          ...
          <Field label="Mapped hierarchy level" helper="Users with this role can be assigned only to org units from this hierarchy level. Specific region/branch/hub values are assigned later on the user form.">
            <Select
              value={form.hierarchyLevelId}
...
export function TenantRolePermissionsPage() {
  ...
  function applyModulePermissionPreset(
    moduleRows: typeof rows,
    preset: "enable_all" | "view_only" | "clear",
  ) {
    setDraftPermissions((existing) => {
      const next = { ...existing };
      moduleRows.forEach(({ moduleFeature, permission }) => {
        const allowedActions = new Set(moduleFeature.actions);
        next[permission.id] = {
          canView: preset === "clear" ? false : allowedActions.has("view"),
          canCreate: preset === "enable_all" ? allowedActions.has("create") : false,
          canEdit: preset === "enable_all" ? allowedActions.has("edit") : false,
          canDelete: preset === "enable_all" ? allowedActions.has("delete") : false,
          canApprove: preset === "enable_all" ? allowedActions.has("approve") : false,
        };
      });
      return next;
    });
            ...
            <Select value={selectedRoleId} onChange={(event) => setSelectedRoleId(event.target.value)}>
              ...
              {selectableRoles.map((role) => (
                <option key={role.id} value={role.id}>
                  {role.name} ({getRoleScopeLevelLabel(role, levelMap)})
                </option>
              ))}
            </Select>
              ...
              {selectedRole ? (
                <>
                  ...
                  <Badge variant="outline">
                    Level: {getRoleScopeLevelLabel(selectedRole, levelMap)}
                  </Badge>
                ...
                <TenantPanel
                  ...
                  action={
                    <div className="flex flex-wrap gap-2">
                      <Button size="sm" variant="outline" onClick={() => applyModulePermissionPreset(moduleRows, "view_only")}>
                        View Only
                      </Button>
                      <Button size="sm" variant="outline" onClick={() => applyModulePermissionPreset(moduleRows, "enable_all")}>
                        Enable All
                      </Button>
                      <Button size="sm" variant="ghost" onClick={() => applyModulePermissionPreset(moduleRows, "clear")}>
                        Clear
                      </Button>
                    </div>
                  }
