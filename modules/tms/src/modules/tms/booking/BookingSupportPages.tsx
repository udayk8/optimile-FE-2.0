...
import { useTenantFinance } from "@/modules/tms/booking/hooks/useTenantFinance";
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
            ...
            <div key={`${booking.id}-actions`} onClick={(event) => event.stopPropagation()} className="flex gap-2">
              ...
              {booking.invoiceId && invoices.some((invoice) => invoice.invoiceId === booking.invoiceId) ? (
                access.can("BOOKING_INVOICING", "VIEW_INVOICE") ? (
                  <Button asChild size="sm" variant="ghost">
                    <Link to={paths.tenantFinanceInvoice(booking.invoiceId)}>View Invoice</Link>
                  </Button>
                ) : null
              ) : null}
            </div>,
        ...
        onRowClick={(rowIndex) => navigate(paths.booking(invoicingBookings[rowIndex].id))}
                        ...
                        <div key={`${invoice.invoiceId}-actions`} className="flex gap-2">
                          <Button
                            ...
                            onClick={() => {
                              ...
                              navigate(paths.tenantFinanceInvoice(invoice.invoiceId));
                            }}
                          >
                            View Invoice
                          </Button>
                    ...
                    <>
                      ...
                      <div className="flex gap-2">
                        {access.can("FINANCE_WORKSPACE", "VIEW_INVOICE") ? (
                          <Button onClick={() => navigate(paths.tenantFinanceInvoice(selectedInvoice.invoiceId))}>View Invoice</Button>
                        ) : null}
                        {access.can("FINANCE_WORKSPACE", "DOWNLOAD") ? (
