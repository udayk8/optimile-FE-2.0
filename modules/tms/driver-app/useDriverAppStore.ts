...
import { areAllDeliveryPodsCaptured, areAllDeliveriesPhysicallyCompleted } from "@/modules/tms/booking/services/booking-engine";
...
export function useDriverAppStore(tenantId: string) {
  ...
  const activeTrips = trips.filter((trip) => ["IN_TRANSIT", "ARRIVED", "POD_PENDING"].includes(trip.booking.status));
  const completedTrips = trips.filter((trip) => ["COMPLETED", "INVOICED"].includes(trip.booking.status));
  ...
  function applyPodCapture(bookingId: string, deliveryId: string, payload: DriverPodFormInput) {
    ...
    if (areAllDeliveriesPhysicallyCompleted(nextDeliveries) && booking.status !== "POD_PENDING" && !areAllDeliveryPodsCaptured(nextDeliveries)) {
      transitionBooking(bookingId, {
        status: "POD_PENDING",
        actor: buildDriverActorLabel(currentDriver),
        note: "All deliveries completed. Waiting for POD collection.",
      });
    }

    if (areAllDeliveryPodsCaptured(nextDeliveries)) {
      if (booking.status !== "POD_PENDING") {
        transitionBooking(bookingId, {
          status: "POD_PENDING",
          actor: buildDriverActorLabel(currentDriver),
          note: "All deliveries completed. Waiting for POD collection.",
