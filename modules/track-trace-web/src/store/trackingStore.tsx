...
interface TrackingStoreContextValue {
  ...
  acknowledgeAlert: (alertId: string, remarks?: string) => Promise<void>
...
export function TrackingStoreProvider({ children }: PropsWithChildren) {
    ...
    if (!activeTrips.length) return
      setDashboardSummary((current) =>
        current
          ? {
            ...current,
            totalActiveTrips: activeTrips.filter((trip) => !['Completed', 'Cancelled'].includes(trip.status)).length,
            inTransitTrips: activeTrips.filter((trip) => ['In Transit', 'At Checkpoint', 'Near Destination', 'Delayed', 'Route Deviated'].includes(trip.status)).length,
            delayedTrips: activeTrips.filter((trip) => trip.delayMinutes > 0 || ['Delayed', 'Offline', 'Route Deviated'].includes(trip.status)).length,
            idleVehicles: activeTrips.filter((trip) => trip.status === 'Idle' || trip.status === 'Stopped').length,
            offlineVehicles: activeTrips.filter((trip) => trip.isOffline).length,
            openAlerts: alerts.filter((alert) => alert.status !== 'Resolved').length,
            averageEtaDelay: activeTrips.length ? Math.round(activeTrips.reduce((total, trip) => total + trip.delayMinutes, 0) / activeTrips.length) : 0,
            onTimePercentage: activeTrips.length ? Math.round((activeTrips.filter((trip) => trip.delayMinutes <= 30).length / activeTrips.length) * 100) : 0,
          }
        : current,
    )
  }, [activeTrips, alerts])
  ...
  const value = useMemo<TrackingStoreContextValue>(
    () => ({
      ...
      refresh,
      acknowledgeAlert: async (alertId, remarks) => {
        const payload = await acknowledgeAlertApi(alertId, {
          acknowledgedBy: 'control.tower@optimile',
          remarks,
        })
        setAlerts((current) =>
          current.map((alert) =>
