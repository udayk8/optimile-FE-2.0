...
export function delayTone(trip: TrackingTrip) {
...
export function ActiveTripsTable({
  ...
  const deviationCell = (row: TrackingTrip) =>
    row.routeDeviationKm > 0 ? (
      <span className="rounded-full bg-warning/10 px-2.5 py-1 text-xs font-semibold text-warning">
        {row.routeDeviationKm} km off route
      </span>
    ) : (
      <span className="rounded-full bg-success/10 px-2.5 py-1 text-xs font-semibold text-success">On route</span>
    )
  ...
  const tableColumns = [
    ...
    },
    {
      ...
      render: (row: TrackingTrip) => (
        <div className="space-y-2">
          <p className="font-semibold text-text">{new Date(row.currentEta ?? row.eta).toLocaleString()}</p>
    ...
    },
    {
      key: 'deviation',
      header: 'Route',
      render: (row: TrackingTrip) => deviationCell(row),
    },
    {
      key: 'lastUpdatedAt',
      header: 'Freshness',
      render: (row: TrackingTrip) => (
        <div className="space-y-1">
          ...
          <p className="text-xs text-gray-500">{new Date(row.lastUpdatedAt).toLocaleTimeString()}</p>
        </div>
      ),
    },
    {
        ...
        <div className="flex flex-col items-start gap-2">
          {onSelectTrip ? (
            <Button
              ...
              onClick={(event) => { event.stopPropagation(); onSelectTrip(row.id) }}
            >
              {row.id === selectedTripId ? 'Viewing' : 'View trip'}
            </Button>
          ) : null}
          <Button asChild size="sm" variant="outline">
            <Link onClick={(event) => event.stopPropagation()} tabIndex={onSelectTrip ? -1 : undefined} to={`${tripBasePath}/${row.id}`}>
              Open trip
            </Link>
          </Button>
        </div>
      ...
      <div>
            ...
            >
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                <div className="rounded-xl bg-gray-50 px-3 py-3">
                  ...
                  <p className="mt-1 text-sm font-semibold text-text">{new Date(row.currentEta ?? row.eta).toLocaleString()}</p>
              ...
              <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                ...
                <Button asChild className="w-full sm:w-auto" size="sm" variant="outline">
                  <Link to={`${tripBasePath}/${row.id}`}>Open trip</Link>
                </Button>
              </div>
            </div>
          ))}
          {hiddenCount > 0 && (
            <p className="px-1 text-sm text-gray-500">+{hiddenCount} more trips — <Link className="font-semibold text-primary hover:underline" to={tripBasePath}>view all</Link></p>
          )}
        </div>
        ...
        <div className="hidden md:block">
          <DataTable
            ...
            getRowClassName={(row) => row.id === selectedTripId ? 'bg-primary/5 ring-1 ring-inset ring-primary/20' : undefined}
            columns={tableColumns}
          />
          {hiddenCount > 0 && (
            <div className="border-t border-gray-200 px-5 py-3">
              <Link className="text-sm font-semibold text-primary hover:underline" to={tripBasePath}>
                +{hiddenCount} more trips — view all active trips
              </Link>
            </div>
          )}
        </div>
