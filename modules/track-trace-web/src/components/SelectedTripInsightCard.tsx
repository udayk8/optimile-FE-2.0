    ...
    <Card className="rounded-none border-0 border-b border-gray-200 bg-gray-50/70 p-4 sm:p-5">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white px-4 py-3">
          ...
          <p className="text-xs text-gray-500">Planned {new Date(trip.plannedEta ?? trip.eta).toLocaleString('en-IN')}</p>
        </div>
        <div className="rounded-2xl border border-gray-200 bg-white px-4 py-3">
          <p className="text-xs font-bold uppercase tracking-wide text-gray-500">Operational status</p>
          <div className="mt-2">
            <TrackingStatusBadge status={trip.status} description={`${trip.routeDeviationKm} km off route · ${trip.idleMinutes ?? 0} min idle`} />
          </div>
        </div>
      </div>
    </Card>
