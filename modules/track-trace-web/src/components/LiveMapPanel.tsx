    ...
    trips.forEach((trip) => {
      const marker = new googleMaps.Marker({
        ...
        icon: {
          ...
          fillColor: trip.isOffline ? '#dc2626' : trip.delayMinutes > 0 ? '#f59e0b' : '#1f4f7a',
          fillOpacity: 1,
          strokeColor: '#ffffff',
          strokeWeight: 1.5,
          rotation: 90,
    ...
    geofenceEvents.slice(0, 12).forEach((event) => {
      const marker = new googleMaps.Marker({
        ...
        icon: {
          ...
          scale: 6,
          fillColor: event.eventType === 'GeofenceEntered' ? '#16a34a' : '#f59e0b',
          fillOpacity: 0.95,
          strokeColor: '#ffffff',
          strokeWeight: 1.5,
        },
      ...
      <div className="grid h-[calc(100vh-15rem)] min-h-[42rem] xl:grid-cols-[22rem,minmax(0,1fr)]">
        <div className="hidden min-h-0 border-r border-gray-200 bg-white/94 backdrop-blur xl:flex xl:flex-col">
          <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
            <div>
              <h3 className="text-[1.75rem] font-extrabold text-text">Active Vehicles ({trips.length})</h3>
              <div className="mt-2 flex flex-wrap gap-2 text-xs font-semibold">
                ...
                {lastUpdatedAt ? (
                  <span className="rounded-full bg-gray-100 px-2.5 py-1 text-gray-600">
                    Updated {new Date(lastUpdatedAt).toLocaleTimeString('en-IN')}
                  </span>
                ) : null}
              </div>
            </div>
        ...
        <div className="flex min-h-0 flex-col bg-white">
          <div className="relative min-h-[24rem] flex-1 overflow-hidden bg-[#667481]">
            {!trips.length ? (
              ...
              <div className="flex h-full items-center justify-center bg-gray-50 px-6 text-center text-sm text-gray-600">
                Google Maps API key is missing. The map shell is ready and will light up once `VITE_GOOGLE_MAPS_API_KEY` is configured.
              </div>
            ) : error ? (
              ...
              <div className="grid gap-4 2xl:grid-cols-[minmax(0,1.15fr),minmax(0,0.9fr),minmax(15rem,0.95fr)]">
                <div className="rounded-2xl border border-gray-200 bg-[#f8fbff] p-4">
                  <div className="flex min-w-0 gap-4">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-blue-100 bg-white text-[#1d4e89]">
                    ...
                    <div className="rounded-2xl border border-gray-200 bg-white p-3.5">
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        ...
                        <span className="rounded-full bg-gray-100 px-2.5 py-1 text-xs font-semibold text-gray-600 shadow-sm">
                          Last ping {new Date(selectedTrip.lastUpdatedAt).toLocaleTimeString('en-IN')}
                        </span>
                      </div>
                      <button className="mt-4 rounded-lg bg-[#1d4e89] px-4 py-2.5 text-[11px] font-bold text-white transition hover:bg-[#003a66]" type="button">
                        View Live Feed
                      </button>
                    </div>
                        ...
                        <div className="rounded-xl bg-gray-50 px-3 py-2.5 text-sm text-gray-600">
                          {latestSourceSwitch ? (
                            <>
                              ...
                              <span className="font-semibold text-text">{new Date(latestSourceSwitch.switchedAt).toLocaleString('en-IN')}</span>.
                            </>
                          ) : (
                ...
                {workbenchView === 'sources' ? (
                  <>
                    ...
                    <div className="rounded-2xl border border-gray-200 bg-white p-3.5">
                      ...
                      <p className="mt-2 text-sm text-gray-600">Switch only when the current feed is stale, faulted, or no longer representative.</p>
                            ...
                            return (
                              <div key={device.id} className="rounded-xl border border-gray-200 bg-gray-50/50 px-3 py-2.5">
                                ...
                                <p className="mt-2 text-xs text-gray-500">
                                  Last seen {device.lastSeenAt ? new Date(device.lastSeenAt).toLocaleString('en-IN') : 'pending'}
                                </p>
                                ...
                                {isSwitchable ? (
                                  <button
                                    ...
                                    onClick={() => {
                                      if (!onSwitchTrackingSource) return
                                      const confirmed = window.confirm(`Switch ${selectedTrip.id} to ${device.label}? Live map visibility will use this source until changed again.`)
                                      if (!confirmed) return
                                      onSwitchTrackingSource(selectedTrip.id, device.id)
                                    }}
                                    type="button"
                                  >
                                    Switch source
                        ...
                        {selectedTripGeofenceEvents.length ? (
                          selectedTripGeofenceEvents.map((event) => (
                            <div key={event.id} className="rounded-xl bg-gray-50 px-3 py-2.5">
                              ...
                              <p className="mt-1 text-xs text-gray-500">{new Date(event.eventTime).toLocaleString('en-IN')}</p>
                            </div>
                          ))
                        ) : (
                          <div className="rounded-xl bg-gray-50 px-3 py-3 text-sm text-gray-600">
