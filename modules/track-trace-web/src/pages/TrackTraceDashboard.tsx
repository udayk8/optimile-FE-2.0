...
export function TrackTraceDashboard() {
  ...
  const activeTripRows = activeTrips.filter((trip) => trip.status !== 'Completed' && trip.status !== 'Cancelled')
  const selectedTrip = activeTripRows.find((trip) => trip.id === selectedTripId) ?? activeTripRows[0]
  const selectedTripAlerts = selectedTrip
    ? alerts.filter((alert) => alert.tripId === selectedTrip.id && alert.status !== 'Resolved')
    : []
  const tripAlertCounts = alerts.reduce<Record<string, number>>((counts, alert) => {
    if (alert.status === 'Resolved') return counts
    counts[alert.tripId] = (counts[alert.tripId] ?? 0) + 1
    return counts
  }, {})
  const alertSeverityCounts = alerts.reduce(
    (counts, alert) => {
      if (alert.status === 'Resolved') return counts
  ...
  const priorityTrips = [...activeTripRows]
    .sort((left, right) => {
      const leftScore =
        (tripAlertCounts[left.id] ?? 0) * 30
        + left.delayMinutes
        + (left.isOffline ? 90 : 0)
        + left.routeDeviationKm * 4
        + ((left.idleMinutes ?? 0) >= 30 ? 10 : 0)
      const rightScore =
        (tripAlertCounts[right.id] ?? 0) * 30
        + right.delayMinutes
        + (right.isOffline ? 90 : 0)
        + right.routeDeviationKm * 4
        + ((right.idleMinutes ?? 0) >= 30 ? 10 : 0)
          ...
          action={
            <div className="flex flex-wrap items-center gap-2">
              <Button asChild variant="outline">
                <Link to={scopedPath('/trips')}>Open active trips</Link>
              </Button>
              <Button asChild>
        ...
        <>
          ...
          <section className="space-y-3">
              <div className="flex flex-wrap items-end justify-between gap-3">
                <div>
                  <p className="text-xs font-extrabold uppercase tracking-widest text-gray-500">Needs attention now</p>
              ...
              <Card className="p-5">
                {priorityTrips.length ? (
                  ...
                  <EmptyPlaceholder
                    compact
                    title="No priority trips"
                    description="Once active trips show delay, alert, or route-health risk, the highest-priority ones will be ranked here."
                  />
                )}
              </Card>
          </section>
