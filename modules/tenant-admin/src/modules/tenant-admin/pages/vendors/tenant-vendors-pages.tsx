...
export function TenantVendorsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {message ? <div className="rounded-xl border border-success/20 bg-success/10 px-4 py-3 text-sm text-success">{message}</div> : null}
          ...
          rows={rows.map((vendor) => [
            ...
            <div key={`${vendor.id}-actions`} className="flex flex-wrap gap-2">
              <Button asChild size="sm" variant="ghost">
                <Link to={paths.vendor(vendor.id)}>View Rate Cards</Link>
              </Button>
  ...
  if (!tenantVendor) {
    return (
      <TenantEmptyState
        ...
        action={<Button asChild><Link to={paths.vendors}>Back to vendors</Link></Button>}
      />
    );
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        action={<Button asChild variant="outline"><Link to={paths.vendors}>Back to vendors</Link></Button>}
      />
    ...
    <div className="space-y-5">
      ...
      >
        <div className="grid gap-4 md:grid-cols-2">
          {error ? <div className="md:col-span-2 rounded-xl border border-danger/20 bg-danger/10 px-4 py-3 text-sm text-danger">{error}</div> : null}
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col gap-2 rounded-xl border border-gray-200 bg-white px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
