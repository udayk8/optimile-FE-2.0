...
export default function VendorMatch({ toast, onNavigate }: any) {
  ...
  const runBatch = () => {
    ...
    setBills((bs) => bs.filter((b) => !batch.includes(b.id)));
    setBatch([]);
    toast(`Batch run: ${n} vendor payment${n > 1 ? "s" : ""} scheduled in one transaction`);
  };
  const batchTotal = bills.filter((b) => batch.includes(b.id)).reduce((s, b) => s + b.billed, 0);
  ...
  return (
    <div>
      <SectionTitle sub="The system auto-checks: Contract Rate = POD exists = Invoice amount. Mismatches are flagged before you pay.">Vendor Bills · 3-Way Match</SectionTitle>

      {batch.length > 0 && (
        <Card className="mb-4 flex items-center justify-between p-4 ring-1 ring-blue-200">
          <div className="text-sm text-slate-600">{batch.length} selected · <span className="font-mono font-semibold text-slate-800">{fmtINR(batchTotal)}</span></div>
          <button onClick={runBatch} className="inline-flex items-center gap-1.5 rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-700">
            <Wallet size={14} />Process batch payment
          </button>
        </Card>
      )}
            ...
            <Card key={b.id} className={`p-5 ${variance ? "ring-1 ring-red-200" : noPod ? "ring-1 ring-amber-200" : ""}`}>
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  {ok && <input type="checkbox" checked={batch.includes(b.id)} onChange={() => toggleBatch(b.id)} className="h-4 w-4 rounded border-slate-300" />}
                  <div>
                    <div className="flex items-center gap-2">
              ...
              {variance && (
                <div className="mt-4 flex items-center gap-2 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700">
                  ...
                  <span>Invoice shows <b className="font-mono">{fmtINR(b.billed)}</b> but contract is <b className="font-mono">{fmtINR(b.contractRate)}</b> — variance of <b>+{b.variance}%</b>. Manual approval required.</span>
                </div>
              )}
