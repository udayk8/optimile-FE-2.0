...
import EnterpriseCommandCenter from "@finance/modules/finance/enterprise/analytics/pages/EnterpriseCommandCenter";
...
export default function Dashboard({ mode, toast }: any) {
  ...
  const podRisk = TRIPS.reduce((s, t) => s + t.revenue, 0);
  const overdue = INVOICES.filter((i) => i.status === "overdue").reduce((s, i) => s + i.amount, 0);
  const pendingBills = VENDOR_BILLS.filter((b) => b.status !== "matched").length;
  const payablesToday = VENDOR_BILLS.filter((b) => b.due === "2026-05-24").reduce((s, b) => s + b.billed, 0);

  const kpis = [
    { label: "Today's Collections", value: 62500, sub: "of ₹1,89,500 due", tone: "amber", icon: CircleDollarSign, trend: "33%" },
    { label: "Revenue at Risk (No POD)", value: podRisk, sub: `${TRIPS.length} trips pending POD`, tone: "red", icon: FileWarning },
    { label: "Overdue Receivables", value: overdue, sub: "across 3 clients", tone: "red", icon: Clock },
    { label: "Invoices Pending Approval", value: null, count: pendingBills, sub: "vendor bills flagged", tone: "amber", icon: ReceiptText },
    { label: "Today's Payables", value: payablesToday, sub: "2 vendor bills due", tone: "blue", icon: Wallet },
    { label: "Cash Position (7-day)", value: 642000, sub: "projected closing balance", tone: "green", icon: Truck },
  ];
          ...
          return (
            <Card key={i} className={`p-5 ring-1 ${toneRing} animate-[fadeUp_.5s_ease] [animation-fill-mode:both]`}>
              <div className="flex items-start justify-between">
                ...
                {k.trend && <Pill tone="amber">{k.trend}</Pill>}
              </div>
