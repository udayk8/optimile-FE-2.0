...
export function DriverAppLayout() {
  ...
  if (!store.session && !location.pathname.endsWith("/driver-app/login")) {
    return <Navigate to={paths.driverLogin} replace />;
  }

  if (store.session && (location.pathname.endsWith("/driver-app") || location.pathname.endsWith("/driver-app/login"))) {
    return <Navigate to={paths.driverDashboard} replace />;
  }

  const tabs = [
    { label: "Dashboard", to: paths.driverDashboard },
    { label: "My Trips", to: paths.driverTrips },
    { label: "Incident Center", to: paths.driverIncidents },
    { label: "Profile", to: paths.driverProfile },
  ];
          ...
          <div className="space-y-3">
            {store.complianceAlerts.length ? (
              store.complianceAlerts.map((alert) => (
                <div key={alert} className="rounded-xl border border-warning/20 bg-warning/10 px-4 py-3 text-sm text-warning">
                  {alert}
                </div>
              ))
            ) : (
            ...
            {store.offlineQueue.length ? (
              <div className="space-y-2">
                {store.offlineQueue.map((action) => (
                  <div key={action.id} className="rounded-xl border border-gray-200 bg-white px-3 py-3 text-sm">
  ...
  if (!trip) {
    return (
      <TenantEmptyState
        ...
        action={<Button onClick={() => navigate(paths.driverTrips)}>Back to My Trips</Button>}
      />
    );
  }
        ...
        <TenantPanel title="Actions" description="Checklist, trip start, arrival, POD, and incidents.">
          <div className="flex flex-wrap gap-2">
            ...
            <Button asChild variant="outline">
              <Link to={paths.driverIncidents}>Incident Center</Link>
            </Button>
          </div>
        ...
        <TenantPanel title="Timeline" description="Trip timeline from booking status history.">
          <div className="space-y-3">
            {tripRecord.booking.statusTimeline
              ...
              .map((event) => (
                <div key={event.id} className="rounded-xl border border-gray-200 bg-white px-4 py-3">
...
export function DriverIncidentCenterPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {message ? (
        <div className="rounded-xl border border-success/20 bg-success/10 px-4 py-3 text-sm text-success">{message}</div>
      ) : null}
          ...
          <div className="space-y-3">
            {store.incidents.length ? (
              store.incidents.map((incident) => (
                <div key={incident.id} className="rounded-xl border border-gray-200 bg-white px-4 py-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
