...
import { BookingRemarksTimeline, BookingStatusTimeline } from "./components/BookingTimeline";
import { useBookingAdminSources } from "./hooks/useBookingAdminSources";
import { useTenantBookings } from "./hooks/useTenantBookings";
import { calculateMarginPercent, getBookingEditability, isBookingDelayCandidate, normalizeBookingId } from "./services/booking-engine";
import {
  buildAddressLookup,
  buildCustomerLookup,
  buildDriverLookup,
...
import { ensureShipmentDocuments } from "./services/shipment-documents";
import type { BookingAssignmentInput, BookingExpenseRecord, BookingPodSnapshot, BookingRemark } from "./types";
...
export function BookingDetailsPage() {
  ...
  if (!booking) {
    return (
      <TenantEmptyState
        ...
        action={
          <Button asChild>
            <Link to={paths.bookings}>Back to booking list</Link>
          </Button>
        }
      />
    );
  ...
  function endLoading() {
    ...
    navigate(paths.bookingDocuments(documentPendingBooking.id));
  }
  ...
  return (
    <div className="space-y-4">
        ...
        action={
          <div className="flex flex-wrap gap-2">
            ...
            <Button asChild variant="outline">
              <Link to={paths.bookings}>Back</Link>
            </Button>
            {getBookingEditability(bookingRecord.status) ? (
              <Button asChild variant="outline">
                <Link to={paths.booking(bookingRecord.id) + "/edit"}>Edit</Link>
              </Button>
            ) : null}
          </div>
        }
      ...
      <div className="grid gap-4 xl:grid-cols-[1.1fr,0.9fr,0.9fr,0.8fr]">
      ...
      <TenantPanel title="Timeline" description="Strict step-by-step progression.">
        <div className="grid gap-3 md:grid-cols-5">
          {timelineSteps.map(([status, label]) => {
            ...
            return (
              <div key={status} className={`rounded-xl border px-4 py-3 ${reached ? "border-success/20 bg-success/10" : "border-gray-200 bg-white"}`}>
        ...
        action={
          <div className="flex flex-wrap gap-2">
            ...
            {["LOADING_COMPLETED", "DOCUMENT_PENDING", "DOCUMENT_COMPLETED"].includes(bookingRecord.status) ? (
              <Button asChild>
                <Link to={paths.bookingDocuments(bookingRecord.id)}>Invoice & E-Way Bill Page</Link>
              </Button>
            ) : null}
            ...
            {shipmentDocuments.lr?.number ? (
              <Button asChild variant="outline">
                <Link to={paths.bookingLR(bookingRecord.id)}>View LR</Link>
              </Button>
            ) : null}
          </div>
        }
                  ...
                  <div className="mt-3 grid gap-3 xl:grid-cols-[1fr_1fr_minmax(260px,320px)]">
                    ...
                    {bookingRecord.status === "DELIVERED" ? (
                      <div className="rounded-xl border border-gray-200 bg-gray-50 p-3">
                        <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-gray-500">POD</p>
            ...
            <div className="space-y-3">
              {(bookingRecord.expenses ?? []).length ? (
                (bookingRecord.expenses ?? []).map((expense) => (
                  <div key={expense.id} className="flex items-center justify-between rounded-xl border border-gray-200 bg-white px-3 py-3 text-sm shadow-sm">
              ...
              {bookingRecord.status === "IN_TRANSIT" ? (
                <div className="grid gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4 md:grid-cols-[1fr_180px_auto]">
...
function HeroStat({ label, value, tone }: { label: string; value: string; tone: "blue" | "indigo" | "emerald" | "amber" }) {
  ...
  return (
    <div className={`rounded-xl border p-4 shadow-sm ${toneClass}`}>
      ...
      <p className="mt-2 text-base font-semibold tracking-[-0.02em] text-text">{value}</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2">
      {sortedRemarks.map((remark) => (
        <div key={remark.id} className="rounded-xl border border-gray-200 bg-gray-50 px-3 py-3">
          ...
          <p className="mt-2 text-sm text-text">{remark.message}</p>
        </div>
      ))}
    </div>
  );
