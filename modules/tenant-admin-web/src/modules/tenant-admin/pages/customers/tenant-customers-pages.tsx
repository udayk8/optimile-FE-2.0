            ...
            return [
              ...
              <div key={`${customer.id}-actions`} className="flex flex-wrap gap-2">
                <Button asChild size="sm" variant="ghost">
                  <Link to={paths.customer(customer.id)}>View Details</Link>
                </Button>
                ...
                {!isFullyConfigured(progress) ? (
                  <Button asChild size="sm" variant="outline">
                    <Link to={paths.customer(customer.id)}>Complete Setup</Link>
                  </Button>
                ) : null}
                {!addressCount ? (
                  <Button asChild size="sm" variant="outline">
                    <Link to={paths.customer(customer.id)}>Add Address</Link>
                  </Button>
                ) : null}
                <Button
                  size="sm"
  ...
  if (!tenantCustomer) {
    return (
      <TenantEmptyState
        ...
        action={
          <Button asChild>
            <Link to={paths.customers}>Back to customers</Link>
          </Button>
        }
      />
    );
...
function getResolvedCustomerAddressName(address: CustomerAddressMasterEntry) {
  ...
  return address.name?.trim() || `${address.city || "Customer"} ${primaryType} Address`;
}
...
function CustomerAddressManagementSection({
  ...
  const [mode, setMode] = useState<"Add Manually" | "Bulk Upload">("Add Manually");
  ...
  return (
    <div className="space-y-4">
      ...
      <Tabs
        tabs={["Add Manually", "Bulk Upload"]}
        active={mode}
        onChange={(value) => {
          setMode(value as "Add Manually" | "Bulk Upload");
          setError("");
          setImportError("");
        }}
      />
      ...
      {!addresses.length ? (
        ...
        <div className="grid gap-3">
          {addresses.map((address) => (
            <div key={address.id} className="rounded-2xl border bg-background/80 px-4 py-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold">
                    {address.name}
                    {address.type.length ? ` (${address.type.join(", ")})` : ""}
                  </p>
                  {address.addressCode ? (
                    <p className="mt-1 text-xs text-muted-foreground">Address Code: {address.addressCode}</p>
                  ) : null}
                  {address.contactCode ? (
                    <p className="mt-1 text-xs text-muted-foreground">Contact Code: {address.contactCode}</p>
                  ) : null}
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {address.type.map((tag) => (
                      <Badge key={`${address.id}-${tag}`} variant="accent">
                        {tag}
                      </Badge>
                    ))}
                  </div>
                  ...
                  {address.gstin ? (
                    <p className="mt-1 text-xs text-muted-foreground">GSTIN: {address.gstin}</p>
                  ) : null}
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button size="sm" variant="outline" onClick={() => deleteDraftAddress(address.id)}>
                    Delete
                  </Button>
                </div>
              </div>
        ...
        {mode === "Add Manually" ? (
          <div className="mt-4 grid gap-4">
            ...
            <div className="flex flex-wrap justify-end gap-3">
              ...
              <Button onClick={saveAddress}>{editingId ? "Save Address" : "Add Address"}</Button>
            </div>
          </div>
        ) : (
          <div className="mt-4 grid gap-4">
