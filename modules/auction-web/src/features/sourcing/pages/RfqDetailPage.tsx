...
export default function RfqDetailPage() {
  ...
  const { rfqs, updateSourcingVendorStatus } = useAppStore()
  ...
  if (!rfq) {
    return (
      <div className="p-8 text-center text-[#64748B]">
        RFQ not found.
        <br />
        <Button variant="outline" className="mt-4" onClick={() => navigate('/auction/sourcing?tab=RFQ')}>Back to Client Hub</Button>
      </div>
    )
  }

  const handleStatusChange = (vendorIdOrEmail: string, newStatus: VendorResponseStatus) => {
    updateSourcingVendorStatus('RFQ', rfq.id, vendorIdOrEmail, newStatus)
  }
  ...
  return (
    <div>
      <HeroCard
        eyebrow="RFQ Event"
        title={rfq.title}
        subtitle="Manage this Request for Quotation and track offline vendor responses."
      />
        ...
        <Card className="h-fit">
          ...
          <CardContent className="space-y-4 text-sm text-[#475569]">
            <div className="rounded-xl border border-[#E5E7EB] p-4">
              <p><span className="font-medium text-[#0F172A]">ID:</span> {rfq.id}</p>
              <p className="mt-2"><span className="font-medium text-[#0F172A]">Status:</span> {rfq.status}</p>
              <p className="mt-2"><span className="font-medium text-[#0F172A]">Deadline:</span> {new Date(rfq.deadline).toLocaleDateString()}</p>
              <p className="mt-2"><span className="font-medium text-[#0F172A]">Template:</span> {rfq.templateFileName || 'None'}</p>
              <p className="mt-2"><span className="font-medium text-[#0F172A]">Created By:</span> {rfq.createdBy}</p>
            </div>

            {rfq.messageToVendor && (
              <div className="rounded-xl border border-[#E5E7EB] bg-[#F8FAFC] p-4">
                <span className="mb-1 block font-medium text-[#0F172A]">Message Sent</span>
                <p className="whitespace-pre-wrap italic">{rfq.messageToVendor}</p>
              </div>
            )}
