...
import type { DriverExpenseType, DriverIncidentType, DriverPodFormInput } from "@/modules/tms/driver-app/types";
import { useDriverAppStore } from "@/modules/tms/driver-app/useDriverAppStore";
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Logged in as {store.currentDriver?.name ?? "Driver"} â€¢ Sync {store.syncStatus}
              </p>
            </div>
              ...
              <div className="space-y-2">
                {store.offlineQueue.map((action) => (
                  <div key={action.id} className="rounded-xl border border-border/60 bg-muted/20 px-3 py-3 text-sm">
                    {action.type.replace(/_/g, " ")} â€¢ {new Date(action.createdAt).toLocaleString()}
                  </div>
                ))}
              </div>
            ) : (
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        eyebrow="Driver App"
        title={trip.booking.bookingId}
        description={`${customer?.name ?? "Customer"} â€¢ ${source?.addressName ?? "Source"} â†’ ${tripRecord.destinations.join(", ")}`}
            ...
            return (
              <div key={delivery.id} className="rounded-[24px] border border-white/80 bg-white/90 p-4 shadow-sm">
                ...
                <div className="mt-4 flex justify-end">
                  <Button onClick={() => savePod(delivery.id)} disabled={!["IN_TRANSIT", "ARRIVED", "POD_PENDING", "COMPLETED"].includes(tripRecord.booking.status)}>
                    Capture POD
                  </Button>
                </div>
              </div>
