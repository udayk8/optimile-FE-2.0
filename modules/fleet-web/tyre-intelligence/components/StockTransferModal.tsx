      ...
      <div className="bg-white rounded-[40px] shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 flex flex-col h-[80vh]">
        <div className="bg-slate-950 px-10 py-8 text-white flex justify-between items-center shrink-0">
          <div>
            ...
            <p className="text-primary-400 text-[10px] font-black uppercase tracking-[0.2em] mt-1">Inter-Hub Asset Movement</p>
          </div>
        ...
        <div className="p-8 border-b border-slate-100 bg-slate-50 flex items-center justify-between gap-8 shrink-0">
           ...
           <div className="flex-1">
              ...
              <select 
                ...
                className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2 text-xs font-bold outline-none focus:ring-2 ring-primary-500/20"
              >
               ...
               {availableTyres.map(tyre => (
                 <div 
                   ...
                   className={`p-4 rounded-2xl border-2 flex justify-between items-center cursor-pointer transition-all ${
                     selectedTyreIds.has(tyre.id) ? 'border-primary-600 bg-primary-50' : 'border-slate-100 hover:border-slate-200'
                   }`}
                 >
                    <div>
                    ...
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${selectedTyreIds.has(tyre.id) ? 'bg-primary-600 border-primary-600 text-white' : 'border-slate-300'}`}>
